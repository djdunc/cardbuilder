   ...
   // } else{
    ...
    if (count($to_delete)>0){
        foreach ($to_delete as $tag){
            ...
            @file_get_contents(BASE_API."cardtags/delete?card_id=".$_POST['card_id']."&tag_id=".$tag_id);
        ...
        foreach ($to_add as $tag){
            ...
            if (!array_search($tag, $_SESSION['c_tags'])){
                ...
                if(array_search($tag, $_SESSION['tags'])){
                    $tag_json= @file_get_contents(BASE_API."cardtags/post?card_id=".$_POST['card_id']."&tag_id=".array_search($tag, $_SESSION['tags']));
                    ...
                    $newCard_json = @file_get_contents(BASE_API."tag/post?name=".$tag."&type=tag");
                    if (isset($newCard_json)){
                        $newCard = json_decode($newCard_json);
                        if (isset($newCard->id)){
                            @file_get_contents(BASE_API."cardtags/post?card_id=".$_POST['card_id']."&tag_id=".$newCard->id);
